...
Realmente $q(x_{t-1}\text{|}x_t, \hat{x}_0)$ y $p(x_{t-1}\text{|}x_t)$ son prácticamente lo mismo, sólo que de forma más explicita, y por lo tanto no habiendo despejado $x_0$, como se demuestra en el proceso de obtener la Fórmula \ref{eq:posterior_clean} desde la Fórmula \ref{eq:posterior}.
