...
1. El Scheduler $ab$
2. El forward process $q(x_t$|$x_{t-1})$
3. La posterior de forward process $q(x_{t-1}|x_t, x_0)$
4. El bakward/reverse process $p_{\theta}(x_{t-1}|x_t)$
...
Y ahora si que sí, con la Fórmula \ref{eq:foward_xoxt} ya tenemos un mecanismo para pasar de cualquier muestra original $x_0$ obtener su versión $x_t$ corrompida hasta llegar al paso $t$ de la cadena de Markov con un único paso. De aquí en adelante, cuando hablemos del Forward process, siempre estaremos hablando de $q(x_t|x_0)$ con la definición de la Fórmula \ref{eq:foward_xoxt}.

### Forward process posterior $q(x_{t-1}|x_t, x_0)$
...
Ahora que tenemos claro como movernos por la cadena tanto hacia adelante como hacia atrás, lo que nos interesa es enseñar a nuestro modelo a retroceder. Es por ello, que con lo que queremos es que $p_{\theta}(x_{t-1}|x_t)$ aprenda a como replicar $q(x_{t-1}|x_t, x_0)$.
...
Básicamente, similar a la Fórmula \ref{posterior}, tenemos que ser capaces de predecir la distribución de la muestra en el paso previo. Para ello, deberíamos de ser capaces de predecir con algún modelo (o dos distintos) tanto la normal $\mu_\theta(x_t, t)$ como varianza $\Sigma_\theta(x_t, t)$. Sin embargo, los creadores del artículo original vieron que se obtenía mejores resultados sin predecir $\Sigma_\theta(x_t, t)$, y total, como al fin y al cabo conocemos eso porque viene establecido por el Scheduler, pues no lo ahorramos (aunque artículos posteriores si que demuestran beneficios en predecir $\Sigma_\theta(x_t, t)$). De esta forma se nos queda así la fórmula anterior: 
...
Con todo esto que hemos dicho, si reconstruimos la Fórmula \ref{backward_mu} con lo desglosado en la Fórmula \ref{mu_prediction} y con la definición de la normal de la Fórmula \ref{normal}, y tenemos en cuenta que $x_{t-1} = p(x_{t-1}|x_t)$ llegamos a la ecuación final para retroceder a lo largo de la cadena:
...
A pesar de que la Fórmula \ref{backwards_final} es la más ampliamente utilizada y que más se explica al hablar de los modelos de difusión, en ocasiones os vais a encontrar que realmente se aplica $q(x_{t-1}|x_t, \hat{x}_0)$ para retroceder a lo largo de la cadena.

Realmente $q(x_{t-1}|x_t, \hat{x}_0)$ y $p(x_{t-1}|x_t)$ son prácticamente lo mismo, sólo que de forma más explicita, y por lo tanto no habiendo despejado $x_9$, como se demuestra en el proceso de obtener la Fórmula \ref{posterior_clean} desde la Fórmula \ref{posterior}.
...
Como hemos visto en la Fórmula \ref{backwards_final} cuando retrocedemos por la cadena añadimos ruido para completar las definiciones de las normales, pero cuando lleguemos al último paso de la cadena, al dar el paso de $x_1$ a $x_0$, no se añade ningún ruido. Tiene sentido si lo piensas: queremos recuperar la muestra original, no queremos añadirle ningún ruido adicional al final
...
Sin embargo, no teneis que tener miedo de esta pedazo de fórmula, porque ya sabemos que realmente lo único que va a determinar que el proceso de reconstrucción esté bien aproximado es una correcta estimación de $\epsilon_\theta(x_t, t)$. Por eso mismo, mediante una limpieza de la Fórmula \ref{elbo}, llegamos a lo siguiente:
