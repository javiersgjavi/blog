...
Los modelos de difusión son un tipo de modelo generativo. Esto significa que se entrena una red para aprender la distribución de unos datos, con el fin de que pueda crear nuevas instancias que encajen dentro de esa distribución. La clave de estos modelos radica en una innovación en su proceso de entrenamiento y generación, más que en la estructura del modelo en sí. Así que, básicamente, los DDPM son una forma diferente de entrenar y generar datos, pero no implican una nueva arquitectura de red.
...
Aunque tradicionalmente se ha visto a los modelos de difusión como uno de los tres pilares fundamentales de los modelos generativos, cada uno con sus fortalezas y debilidades específicas, la línea entre estos pilares está empezando a ser eliminada por los modelos de difusión. La introducción de mejoras en la velocidad de generación, como las Denoising Diffusion Implicit Models (DDIM), evidencia un esfuerzo por superar las limitaciones de estos modelos y expandir sus aplicaciones prácticas.
...
$$
\begin{equation}
  q(x_t|x_{t-1}) = \mathcal{N}(x_t; \underbrace{\sqrt{1-\beta_t}x_{t-1}}{\mu}, \underbrace{\beta_t I}{\sigma^2})
  \label{eq:foward}
\end{equation}
$$
